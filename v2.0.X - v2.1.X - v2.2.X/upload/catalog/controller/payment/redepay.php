    ...
    public function index() {
		...
		if(version_compare(VERSION, "2.2.0.0") < 0) {
            if (file_exists(DIR_TEMPLATE . $this->config->get("config_template") . "/template/payment/redepay.tpl")) {
                return $this->load->view($this->config->get("config_template") . "/template/payment/redepay.tpl", $data);
            }
			else {
                return $this->load->view("default/template/payment/redepay.tpl", $data);
            }
        }
		else {
            return $this->load->view("payment/redepay", $data);
        }
    }
